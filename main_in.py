...
from utils import AverageMeter, RecorderMeter, time_string, convert_secs2time, _ECELoss, load_dataset_in, plot_mi, plot_ens
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_loader, test_loader = load_dataset_in(args)
...
def adjust_learning_rate(optimizer, var_optimizer, epoch, args):
    lr = args.learning_rate * args.batch_size * args.world_size / 256 * (0.1 ** (epoch // 30 + int(epoch>=80)))
    slr = args.log_sigma_lr * args.batch_size * args.world_size / 256 * (0.1 ** (epoch // 30 + int(epoch>=80)))
