...
def _to_bayesian(input):
    ...
    if isinstance(input, (Linear, Conv2d, BatchNorm2d)):
        if isinstance(input, (Linear)):
            output = BayesLinearEMP(input.in_features, input.out_features, input.bias)
...
def _to_deterministic(input):
    ...
    if isinstance(input, (BayesLinearEMP, BayesConv2dEMP, BayesBatchNorm2dEMP)):
        if isinstance(input, (BayesLinearEMP)):
            output = Linear(input.in_features, input.out_features, input.bias)
        elif isinstance(input, (BayesConv2dEMP)):
